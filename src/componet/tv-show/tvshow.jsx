...
export default function TvShow() {
  let [tvShowDisplay, setTvShowDisplay] = useState([]);
  ...
  async function getdata(pageNumber = 1) {
    let response = await axios.get(
      `https://api.themoviedb.org/3/tv/popular?api_key=3772072d6fd76a53c64b2c06092d788d&language=en-US&page=${pageNumber}`
    );
    setNextPageNumber(Number(pageNumber) + 1);
    setPrePageNumber(pageNumber - 1);
    setTvShowDisplay(response.data.results);
  }
  function nextPage() {
  ...
  return (
    <>
      <div className="trending row gx-3">
        {tvShowDisplay.map((tv, i) => (
          <div key={i} className="col-md-2 col-sm-12 trending-display">
            <Link className="link" to={`/tvdetails/${tv.id}/tv`}>
